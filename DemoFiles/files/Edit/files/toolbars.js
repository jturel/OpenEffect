...
function TBButtonMouseUp() {
  ...
  // Initialize tbEventSrcElement so that the user's onClick handler can find out where the event is coming from
...

{
  ...
  // Add an onmouseover handler to the tbContentElement. We need this for when the DHTML Edting control is the content element. IE doesn't give the toolbars onmouseout events in that case. 
...
function TBRebuildToolbar(toolbar, rebuildMenus){
  ...
  if (false == toolbarFound) { 	
    ...
    if ((s = TBInitToolbar(toolbar)) != TB_STS_OK) alert("Toolbar: " + toolbar.id + " failed to initialize. Status: " + s);
    ...
    for (i=0; i<document.body.all.length; i++) if (document.body.all[i].className == "tbMenu") TBRebuildMenu(document.body.all[i], rebuildMenus);

    // Populate the toolbar with its contents
    if ((s = TBPopulateToolbar(toolbar)) != TB_STS_OK) alert("Toolbar: " + document.body.all[i].id + " failed to populate. Status: " + s);
