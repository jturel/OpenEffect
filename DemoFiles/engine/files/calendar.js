...
function buildCalendar(iMonth, iYear, iDay){
  ...
  for( c=1; c < 43; c++ ){
    ...
    if( (c >= fromDay) && (d <= stopDay) ){
      if(d==iDay && (iMonth-1) == CurDate.getMonth() && iYear == CurDate.getFullYear()) oCell.className = "calendarCellCurrent";
      else oCell.className = "calendarCell";
      oCell.id = "c_"+d;
      oCell.innerHTML=d;
      d++;
    }
    ...
    for(i = 0; i < 6; i++ ) {
      for(j = 0; j < 7; j++ ) {
        if( doc["CalendTab"].rows[i].cells[j].id.substr(0,2)=='c_'){
          ...
          } else doc["CalendTab"].rows[i].cells[j].className = "calendarCell";
        }
      }
    }
